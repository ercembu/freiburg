...
class DNGO(nn.Sequential):
    ...
    def fit_blr_model(self, mu_pre: np.array, Sigma_pre: np.array,
        ...
        x_train_last_hidden_layer = self.last_hidden_layer_features(x_train)

        self.blr = BLR(mu_pre=mu_pre, sigma_pre=Sigma_pre, noise=noise, bias=True)
        self.mu_post, self.Sigma_post = self.blr.linreg_bayes(x_train_last_hidden_layer, y_train)
        # END TODO ########################
    ...
    def predict_mean_and_std(self, x: torch.Tensor) -> Tuple[np.array, np.array]:
        ...
        # START TODO ########################
        # Call last_hidden_layer_features(...) (remember to use detach and convert it to numpy)
        x_last_hidden_layer = self.last_hidden_layer(x).detach().numpy()
        x_mean, x_std = self.blr.posterior_predictive(x_last_hidden_layer)
        # END TODO ########################
        return x_mean, x_std
