...
def plot_bayes_linear_regression(lbr_1d: BLR, X_train: np.array,
    ...
    alpha = 1.0
    y = weight * x
    plt.plot(x, y, c='r', alpha=alpha, label='Prior model')
    alpha = samples_pdf_prior[0] * 0.25 / pdf_max_val
    plt.plot(x, samples_prior[0] * x, c='g', alpha=alpha, label='Different prior models sampled')

    for i in range(1, len(samples_prior)):
        weight = samples_prior[i]
        alpha = samples_pdf_prior[i] * 0.25 / pdf_max_val
        y = weight * x
        plt.plot(x, y, c='g', alpha=alpha)
    # END TODO ########################
    ...
    plt.plot(x, y, c='fuchsia', alpha=alpha, label='Posterior model')
    alpha = samples_pdf_post[0] * 0.25 / pdf_max_val
    plt.plot(x, samples_prior[0] * x, c='b', alpha=alpha, label='Different posterior models')

    for i in range(1, len(samples_post)):
        weight = samples_post[i]
        alpha = samples_pdf_post[i] * 0.25 / pdf_max_val
        y = weight * x
        plt.plot(x,y, c='b', alpha=alpha)
    # END TODO ########################
