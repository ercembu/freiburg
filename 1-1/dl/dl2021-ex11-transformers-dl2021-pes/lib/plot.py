...
def show_attention_on_input_image(net) -> None:
    ...
    for i, (data, target) in enumerate(dataloader):
        if i in [
            ...
            # START TODO #############
            norm = torch.linalg.norm(gradient - data, dim = 1, ord = 2)
            normalized_gradient = torch.linalg.norm(norm, dim = 0, ord = 1).detach().numpy()
            normalized_gradient /= np.max(normalized_gradient)
            # END TODO #############
    ...
    for i, (data, target) in enumerate(dataloader):
        if i in [
            ...
            try:
                ...
                # START TODO #############
                # use the gradient w.r.t. the input to augment the original data to get an adversarial example
                adversarial_example = orig_data + gradient
                # END TODO #############
...
def plot_attention():
    ...
    attention.load_state_dict(torch.load(os.path.join("results", "model.pth")))
    attention.eval()
    predictions, attention = attention(x)
    ...
    plot_kwargs = {
        ...
        "yticklabels": ["Number 0", "Number 1", "Number 2"],
        "cmap": "plasma",
        "cbar": True,
    ...
    for att in attention:
        seaborn.heatmap(att, **plot_kwargs)
    # END TODO #############
