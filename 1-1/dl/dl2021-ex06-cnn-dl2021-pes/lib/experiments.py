...
def create_conv_model() -> Module:
    ...
    conv_model = Sequential(Conv2d(1, n_filters_conv1, kernel_size, stride, padding), ReLU(),
                            Conv2d(n_filters_conv1, n_filters_conv2, kernel_size, stride, padding), ReLU(), Flatten(),
                            Linear(490, 10))
    # END TODO ################
    return conv_model
...
def run_shifted_conv_experiment(max_datapoints: int = 10000):
    ...
    for x in x_val_shifted:
        for i in range(len(x)):
            x[i] = np.pad(x[i],((shift,0),(shift,0)), "constant")[:x[i].shape[0], :x[i].shape[1]]
    ...
    mlp_model = Sequential(Linear(x_val_shifted.shape[1]*x_val_shifted.shape[2]*x_val_shifted.shape[3], linear_units),
            ReLU(), Linear(linear_units, 10))
    optimizer = SGD(mlp_model.parameters(), lr=learning_rate, momentum=momentum)
    x_val_shifted_mlp = x_val_shifted.reshape((x_val_shifted.shape[0],-1))
    x_val_mlp = x_val.reshape((x_val.shape[0],-1))
    train_results = train(mlp_model, loss_fn, optimizer, x_train.reshape((x_train.shape[0],-1)), y_train,
            x_val_mlp, y_val, num_epochs=num_epochs,
                          batch_size=batch_size)
    # END TODO ################
