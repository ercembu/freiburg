...
class VAE(nn.Module):
    ...
    def __init__(self, in_channels: int = 1, in_height: int = 28, in_width: int = 28, hidden_size: int = 100,
        ...
        self.encode_layer = nn.Linear(input_size, hidden_size)
        self.mu_layer = nn.Linear(hidden_size, latent_size)
        self.log_layer = nn.Linear(hidden_size, latent_size)
        self.latent_hidden = nn.Linear(latent_size, hidden_size)
    ...
    def encode(self, x: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        # START TODO ########################
        h1 = self.encode_layer(x)
        h1 = self.activation(h1)
        
        mu = self.mu_layer(h1)
        logvar = self.log_layer(h1)
        # END TODO ########################
        return mu, logvar
