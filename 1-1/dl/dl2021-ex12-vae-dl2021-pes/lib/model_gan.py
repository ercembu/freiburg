...
class Generator(nn.Module):
    ...
    def __init__(self, channels_multiplier: int = 32, num_input_channels: int = 3, latent_size: int = 64):
        ...
        # layers = [ ... ]
        layers = [nn.ConvTranspose2d(latent_size, channels_multiplier*8, 4, bias=False),
                nn.BatchNorm2d(channels_multiplier*8),
                activation_g(),
                nn.ConvTranspose2d(channels_multiplier*8, channels_multiplier *4, 4, 2, 1, bias=False),
                nn.BatchNorm2d(channels_multiplier *4),
                activation_g(),
                nn.ConvTranspose2d(channels_multiplier*4, channels_multiplier *2, 4, 2, 1, bias=False),
                nn.BatchNorm2d(channels_multiplier *2),
                activation_g(),
                nn.ConvTranspose2d(channels_multiplier*2, channels_multiplier, 4, 2, 1, bias=False),
                nn.BatchNorm2d(channels_multiplier),
                activation_g(),
                nn.Conv2d(channels_multiplier, num_input_channels, 3, 1, 1, bias=False),
                nn.Tanh()] 
...
class Discriminator(nn.Module):
    ...
    def __init__(self, channels_multiplier=64, num_input_channels=3):
        ...
        # layers = [ ... ]
        layers = [
                nn.Conv2d(num_input_channels, channels_multiplier, 4, 2, 1, bias=False),
                activation_d(),

                nn.Conv2d(channels_multiplier, channels_multiplier*2, 4, 2, 1, bias=False),
                nn.BatchNorm2d(channels_multiplier*2),
                activation_d(),

                nn.Conv2d(channels_multiplier*2, channels_multiplier*4, 4, 2, 1, bias=False),
                nn.BatchNorm2d(channels_multiplier*4),
                activation_d(),

                nn.Conv2d(channels_multiplier*4, 1, 4, 1, 0, bias=False),
                nn.Sigmoid()
                ]
        # END TODO ##########
        self.layers = nn.ModuleList(layers)
