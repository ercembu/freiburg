...
def train_noise_removal_model(hidden_size=40, shift=10, lr=0.01, num_epochs=101, batch_size=10, plot=True) -> None:
    """
    ...
    for epoch in range(num_epochs):
        # START TODO #############
        # training loop here
        for batch_idx in range(train_sequences.shape[0] // batch_size):
            optimizer.zero_grad()
            batch = noisy_train_sequences[batch_idx * batch_size:(batch_idx + 1) * batch_size]
            output = model(batch)
            loss = loss_fn(output, train_sequences[batch_idx * batch_size:(batch_idx + 1) * batch_size])
            loss.backward()
            optimizer.step()
        # END TODO #############
...
def run_hpo() -> None:
    ...
    # START TODO
    train_noise_removal_model(hidden_size=40, shift=10, lr=0.01, num_epochs=200, batch_size=10, plot=True)
    # END TODO
